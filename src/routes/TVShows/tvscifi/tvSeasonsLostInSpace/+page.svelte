<script>
	...
	onMount(async () => {
		let addr = 'http://192.168.0.99:8080/scifi/lostinspace/01';
		const res = await fetch(addr);
		data1 = await res.json();

		let addr2 = 'http://192.168.0.99:8080/scifi/lostinspace/02';
		const res2 = await fetch(addr2);
		data2 = await res2.json();

		let addr3 = 'http://192.168.0.99:8080/scifi/lostinspace/03';
